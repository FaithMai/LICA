...
from modules.critics.tc_lica import TCLICACritic
...
class TCLICALearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        mix_loss = self.critic(mac_out, batch["state"][:, :-1], mask)
    ...
    def train_critic_td(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # Optimise critic
        target_q_vals = self.target_critic(actions, batch["state"], mask)[:, :]
        ...
        q_t = self.critic(actions[:, :-1], batch["state"][:, :-1], mask)
